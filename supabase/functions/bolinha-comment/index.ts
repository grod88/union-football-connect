...
serve(async (req) => {
  ...
  try {
    const {
      ...
      custom_prompt,
      generate_audio,
      // Direct mode: skip AI, use provided text/emotion
      direct_text,
      direct_emotion,
    } = await req.json();
    ...
    // Direct mode: skip AI entirely
    let commentText = "";
    let emotion = "neutro";

    // Sanitize text for TTS (remove problematic chars)
    const sanitizeForTTS = (text: string): string => {
      return text
        .replace(/[""'']/g, '"')  // Normalize quotes
        .replace(/[…]/g, '...')   // Normalize ellipsis
        .replace(/[—–]/g, '-')    // Normalize dashes
        .replace(/[^\w\s.,!?;:'"()\-áàâãéèêíìîóòôõúùûçÁÀÂÃÉÈÊÍÌÎÓÒÔÕÚÙÛÇ]/g, '') // Remove special chars
        .replace(/\s+/g, ' ')     // Normalize whitespace
        .trim();
    };

    if (direct_text) {
      // Use provided text/emotion directly (no AI)
      commentText = sanitizeForTTS(direct_text);
      emotion = direct_emotion || "neutro";
      const validEmotions = ["neutro", "gol", "bravo", "analise", "sarcastico", "tedio"];
      if (!validEmotions.includes(emotion)) {
        emotion = "neutro";
      }
    } else {
      // AI mode: build prompt and call Claude
      let userPrompt = "";
      ...
      if (!claudeResponse.ok) {
        ...
        return new Response(
          JSON.stringify({ error: "AI generation failed", details: errText, status: claudeResponse.status }),
          { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }
        );
      }
    ...
    // For large audio (>200KB base64), upload to storage and send URL
    let audioUrl: string | null = null;
    let broadcastAudio: string | null = audioBase64;
    const MAX_BROADCAST_SIZE = 200000; // 200KB limit for Realtime

    if (audioBase64 && audioBase64.length > MAX_BROADCAST_SIZE) {
      console.log(`Audio too large for broadcast (${audioBase64.length} chars), uploading to storage...`);
      try {
        // Extract base64 data
        ...
        const filename = `audio/bolinha-${Date.now()}.mp3`;
        ...
        if (uploadError) {
          ...
          broadcastAudio = null; // Don't send base64, send URL instead
    ...
    await supabase.from("bolinha_messages").insert({
      ...
      audio_url: audioUrl || (audioBase64 ? "generated" : null),
